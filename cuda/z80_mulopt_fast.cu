...
// Reduced 14-op pool (7 never-used ops removed for 38x speedup at len-9)
// Removed: SLA A (=ADD A,A but 8T), SRA A, RLC A, RRC A (CB=slower), OR A, SCF, EX AF,AF'
...
#define OP_SRL_A    8   // SRL A    (8T) — logical shift right, bit7=0
#define OP_RLA      9   // RLA      (4T) — rotate left through carry
#define OP_RRA     10   // RRA      (4T) — rotate right through carry
#define OP_RLCA    11   // RLCA     (4T) — rotate left circular
#define OP_RRCA    12   // RRCA     (4T) — rotate right circular
#define OP_NEG     13   // NEG      (8T) — negate A
// Dead ops (indices >= NUM_OPS, never generated by decode_seq)
#define OP_SLA_A   14
#define OP_SRA_A   15
#define OP_RLC_A   16
#define OP_RRC_A   17
...
__constant__ uint8_t opCost[21] = {  // 21 entries for exec_op, only 0-13 generated
    4,4,4,4,4,4,4,4,  // ADD/ADC/SBC/LD B,A
    8,                 // SRL A
    4,4,4,4,           // RLA/RRA/RLCA/RRCA
    8,                 // NEG
    // Dead entries (never reached):
    8,8,8,8,4,4,4     // SLA/SRA/RLC/RRC/OR/SCF/EX
};
...
static const char *opName(uint8_t op) {
    static const char *names[] = {
        ...
        "RLA", "RRA", "RLCA", "RRCA",
        "NEG",
        "SLA A", "SRA A", "RLC A", "RRC A", "OR A", "SCF", "EX AF,AF'"
    };
    return op < 21 ? names[op] : "?";
}
